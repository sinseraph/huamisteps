...
def main(_user, _passwd, min_1, max_1):
    ...
    print("set random steps(" + str(min_1) + "~" + str(max_1) + ")")
    if user == '' or password == '':
        print("用户名或密码填写有误！")
        return
    ...
    url = f'https://api-mifit-cn.huami.com/v1/data/band_data.json?&t={current_time}'
    head = {
        "apptoken": app_token,
    ...
    # print(response)
    result = f"[{now}]\nAccount：{user[:2]}****{user[-6:]}\nset steps（{step}）[" + response['message'] + "]\n"
    print(result)
    return result
